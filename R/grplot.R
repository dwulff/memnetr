...
graph_plot = function(g,
  ...
  apply(cbind(lyt,txt.pos),1,
        function(x) lines(c(x[1], x[3]),
                          c(x[2], x[4]),
                          lty=3,lwd=(lwd-.5)**.5,col='grey75'))
...
graph_plot2 = function(g,
  ...
  apply(cbind(lyt,txt.pos),1,
        function(x) lines(c(x[1], x[3]),
                          c(x[2], x[4]),
                          lty = 3,
                          lwd = (lwd - .5) ** .5, col = 'grey75'))
  points(txt.pos, pch = 16, lwd = 1, col = 'white', cex = 1.5)

  # expand edge line weight
  if(length(lwd) != nrow(edg)){
    lwd = rep(lwd[1], nrow(edg))
    }
